...
#%%
    def search_space(inputs):
        ...
        for i in range(len(np.array(common_ss.exterior.coords))):
            common_ss_exterior.append([np.array(common_ss.exterior.coords)[i][0],
...
#        around individual centroids as outputs using k-means clustering.
        ...
        for i in range(len(pbp_kmeans)):
            pbp_kmeans_coords.append([pbp_kmeans[i].x,
                                      pbp_kmeans[i].y,
                                      agv])
...
#       around each centroid as outputs using the OPTICS algoritm
        ...
        for i in range(len(point_data_discrete)):
            point_data_discrete_2d.append([point_data_discrete[i][0],point_data_discrete[i][1]])
        ...
        for i in range(len(clusters_optics)):
            centroids_optics[i] = [np.mean(np.array(clusters_optics[i])[:,0]),
                                   np.mean(np.array(clusters_optics[i])[:,1])]       
        centroids_ss_optics = []
        for i in range(len(centroids_optics)):
            centroids_ss_optics.append(Point(centroids_optics[i][0],
                                           centroids_optics[i][1],
                                           0).buffer(buffer_out+200)) 
...
#            pbp_optics = final_ss_optics[i].union(final_ss_optics[i+1])
        ...
        for i in range(len(pbp_optics)):
            pbp_optics_coords.append([pbp_optics[i].x,
                                      pbp_optics[i].y,
                                      agv])
        return pbp_optics_coords, centroids_optics, clusters_optics, pbp_optics,final_ss_optics, centroids_ss_optics
...
#       the best. returns allocated task points
        ...
        for i in allocate_combo:
            common = list(set(task_allocation_indices[i[0]]).intersection(task_allocation_indices[i[1]]))
            if len(common) != 0:
                if len(task_allocation_indices[i[0]]) > len(task_allocation_indices[i[1]]):
...
#       and returns an organized dictionary of them
        ...
        perc = ( len(all_reachable_points_discrete)+len(all_reachable_points_coverage) )/(len(point_data_coverage)+len(point_data_discrete))
...
#       the considered base placement and allocation
        ...
        for i in range(len(multi_traj)):
            for j in range(len(multi_traj[i])):
                  torq_temp.append(sum(robot.rne(multi_traj[i].q[j], np.zeros((6,)), np.zeros((6,)))))  
            torques[i] = torq_temp
            torq_temp = []
        max_torques = {}
        for i in range(len(multi_traj)):
...
#       bounding boxes around the I-beams
        ...
        A = np.array([(x -1500)*c- (y-500)*s ,(x-1500)*s + (y -500)*c, z])
        B = np.array([(x +1500)*c- (y-500)*s ,(x+1500)*s + (y -500)*c, z])
        C = np.array([(x +1500)*c- (y+500)*s ,(x+1500)*s + (y +500)*c, z])
        D = np.array([(x -1500)*c- (y+500)*s ,(x-1500)*s + (y +500)*c, z])
        ...
        E = np.array([(x -1500)*c- (y-500)*s ,(x-1500)*s + (y -500)*c, z+100])
        F = np.array([(x +1500)*c- (y-500)*s ,(x+1500)*s + (y -500)*c, z+100])
        G = np.array([(x +1500)*c- (y+500)*s ,(x+1500)*s + (y +500)*c, z+100])
        H = np.array([(x -1500)*c- (y+500)*s ,(x-1500)*s + (y +500)*c, z+100])
        
        I = np.array([(x -1500)*c- (y-500)*s ,(x-1500)*s + (y -500)*c, z+800])
        J = np.array([(x +1500)*c- (y-500)*s ,(x+1500)*s + (y -500)*c, z+800])
        K = np.array([(x +1500)*c- (y+500)*s ,(x+1500)*s + (y +500)*c, z+800])
        L = np.array([(x -1500)*c- (y+500)*s ,(x-1500)*s + (y +500)*c, z+800])
        #
        M = np.array([(x -1500)*c- (y-500)*s ,(x-1500)*s + (y -500)*c, z+900])
        N = np.array([(x +1500)*c- (y-500)*s ,(x+1500)*s + (y -500)*c, z+900])
        O = np.array([(x +1500)*c- (y+500)*s ,(x+1500)*s + (y +500)*c, z+900])
        P = np.array([(x -1500)*c- (y+500)*s ,(x-1500)*s + (y +500)*c, z+900])
        top_surface = Polygon([M,N,O,P])
        #
        Q = np.array([(x -1500)*c- (y+50)*s ,(x-1500)*s + (y +50)*c, z + 100])
        R = np.array([(x -1500)*c- (y-50)*s ,(x-1500)*s + (y -50)*c, z + 100])
        S = np.array([(x -1500)*c- (y+50)*s ,(x-1500)*s + (y +50)*c, z + 800])
        T = np.array([(x -1500)*c- (y-50)*s ,(x-1500)*s + (y -50)*c, z + 800])
        #
        U = np.array([(x +1500)*c- (y+50)*s ,(x+1500)*s + (y +50)*c, z + 100])
        V = np.array([(x +1500)*c- (y-50)*s ,(x+1500)*s + (y -50)*c, z + 100])
        W = np.array([(x +1500)*c- (y+50)*s ,(x+1500)*s + (y +50)*c, z + 800])
        X = np.array([(x +1500)*c- (y-50)*s ,(x+1500)*s + (y -50)*c, z + 800])
        ...
        def random_points_within(poly, num_points):
            ...
            while len(points) < num_points:
                random_point = Point([random.uniform(min_x, max_x), random.uniform(min_y, max_y), 900])
